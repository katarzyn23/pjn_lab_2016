...
def markow_from_file(file_name):
	with open(file_name, 'r') as ngrams_file:
		markow_dict = {}
		ngrams=ngrams_file.read().replace('\n', '')
		ngrams_list = ngrams.split()
		markow_dict = markow_dict_from_list(ngrams_list)
		print markow_dict
	return markow_dict
...
def markow_from_string(string_name):
	markow_dict = {}
...
def markow_from_pap():
	...
	simple_article = ''
	i = 1
	with codecs.open('pap_small.txt', 'r', 'utf-8') as pap_file:
		for line in pap_file:
			if line[0] != '#':
				simple_article = simple_article + line	
			else:
				tmp_dict = markow_from_string(simple_article)
...
ngrams_markow = markow_from_file('ngrams.txt')
